...
def run_ssd_2tb_pcie_gen4_x4():
    ...
    # Menampilkan gambar
    image_path = os.path.join(CURRENT_PATH, 'Analysis', 'SSD', 'ssd_2tb_pcie_gen4_x4', 's-l1600.webp')
                              
    data = mpimage.imread(image_path)
    st.image(data, caption='EDA SSD 2TB PCIE Gen4x4')
    df = pd.read_csv('./Analysis/SSD/ssd_2tb_pcie_gen4_x4/2tb_pcie_gen4_x4.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date')

    st.write('# Exploratory Data Analysis untuk komponen SSD 2TB PCIE Gen4x4')
    ...
    fig.suptitle('Visualisasi ACF dan PACF dari data pelatihan harga rata-rata SSD 2TB PCIE Gen4x4', fontsize=16)
    ...
    perform_adf_test(df_avg_price_train['avg'], series_name="Data Harga Rata-rata SSD 2TB PCIE Gen4x4")

    st.subheader('4. Data differencing')

    train_diff = df_avg_price_train.diff().dropna()
    perform_adf_test(train_diff['avg'], series_name="Data Harga Rata-rata SSD 2TB PCIE Gen4x4 melalui 1x diferensiasi")
