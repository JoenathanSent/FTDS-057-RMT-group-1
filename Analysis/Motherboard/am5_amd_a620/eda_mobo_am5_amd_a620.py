...
def run_am5_amd_a620():
    ...
    # Menampilkan gambar
    image_path = os.path.join(CURRENT_PATH, 'Analysis', 'Motherboard', 'am5_amd_a620', 'msi_msi_pro_a620m-e_motherboard_amd_a620_socket_am5_ddr5_matx_full01_hfcsp05i.webp')
                              
    data = mpimage.imread(image_path)
    st.image(data, caption='EDA AM5 AMD A620')
    df = pd.read_csv('./Analysis/Motherboard/am5_amd_a620/am5_amd_a620.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date')

    st.write('# Exploratory Data Analysis untuk komponen Motherboard AM5 AMD A620')
    ...
    fig.suptitle('Visualisasi ACF dan PACF dari data pelatihan harga rata-ratad Motherboard AM5 AMD A620', fontsize=16)
    ...
    st.write('- PACF bersifat cuts off setelah lag 3, berarti nilai p = 3, dengan alternatif nilai p adalah 5, 8, 14, dan 28')
    ...
    perform_adf_test(df_avg_price_train['avg'], series_name="Data Harga Rata-ratad Motherboard AM5 AMD A620")

    st.subheader('4. Data differencing')

    train_diff = df_avg_price_train.diff().dropna()
    perform_adf_test(train_diff['avg'], series_name="Data Harga Rata-ratad Motherboard AM5 AMD A620 melalui 1x diferensiasi")
