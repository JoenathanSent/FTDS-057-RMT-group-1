...
def run_ram_16gb_ddr5():
    ...
    # Menampilkan gambar
    image_path = os.path.join(CURRENT_PATH, 'Analysis', 'RAM', 'ram_16gb_ddr5', 'DDR5-4800MHz-VenomRX.png')
                              
    data = mpimage.imread(image_path)
    st.image(data, caption='EDA RAM 16GB DDR5')
    df = pd.read_csv('./Analysis/RAM/ram_16gb_ddr5/16gb_ddr5.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date')

    st.write('# Exploratory Data Analysis untuk komponen RAM 16GB DDR5')
    ...
    fig.axes[0].set_title('Data Rata-rata Harga RAM 16GB DDR5')
    fig.axes[1].set_title('Tren Rata-rata Harga RAM 16GB DDR5')
    ...
    fig.suptitle('Visualisasi ACF dan PACF dari data pelatihan harga rata-rata RAM 16GB DDR5', fontsize=16)
    ...
    st.write('- PACF bersifat cuts off setelah lag 3, berarti nilai p = 3, dengan alternatif nilai p adalah 6, 8, 9, 18, dan 23')
    ...
    perform_adf_test(df_avg_price_train['avg'], series_name="Data Harga Rata-rata RAM 16GB DDR5")

    st.subheader('4. Data differencing')

    train_diff = df_avg_price_train.diff().dropna()
    perform_adf_test(train_diff['avg'], series_name="Data Harga Rata-rata RAM 16GB DDR5 melalui 1x diferensiasi")
