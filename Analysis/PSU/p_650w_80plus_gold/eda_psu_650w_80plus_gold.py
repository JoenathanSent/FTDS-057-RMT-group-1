...
def run_650w_80plus_gold():
    # Membuat title 
    st.title('DealSense')

    # Membuat sub header
    st.subheader('Halaman ini berisi Exploratory Data Analysis (EDA) mengenai dataset harga komponen PSU Merk 650W 80+ Gold')

    # Menampilkan teks
    st.write('Halaman ini menampilkan eksplorasi data dari data harga komponen komputer PSU 650W 80+ Gold sejak tanggal 21 Agustus 2021 sampai dengan 21 Agustus 2026.')
    # Menampilkan gambar
    image_path = os.path.join(CURRENT_PATH, 'Analysis', 'PSU', 'p_650w_80plus_gold', 'brd-44261_msi-mag-a650gl-psu-650w-80-gold-fully-modular_full01-456f84bb.webp')
                              
    data = mpimage.imread(image_path)
    st.image(data, caption='EDA PSU 650W 80+ Gold')
    df = pd.read_csv('./Analysis/PSU/p_650w_80plus_gold/650w_80plus_gold.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date')

    st.write('# Exploratory Data Analysis untuk komponen PSU 650W 80+ Gold')
    ...
    fig.axes[0].set_title('Data Rata-rata Harga PSU 650W 80+ Gold')
    fig.axes[1].set_title('Tren Rata-rata Harga PSU 650W 80+ Gold')
    ...
    fig.suptitle('Visualisasi ACF dan PACF dari data pelatihan harga rata-rata PSU 650W 80+ Gold', fontsize=16)
    ...
    perform_adf_test(df_avg_price_train['avg'], series_name="Data Harga Rata-rata PSU 650W 80+ Gold")

    st.subheader('4. Data differencing')

    train_diff = df_avg_price_train.diff().dropna()
    perform_adf_test(train_diff['avg'], series_name="Data Harga Rata-rata PSU 650W 80+ Gold melalui 1x diferensiasi")
