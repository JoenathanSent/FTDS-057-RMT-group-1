...
def run_750w_80plus_gold():
    ...
    # Menampilkan gambar
    image_path = os.path.join(CURRENT_PATH, 'Analysis', 'PSU', 'p_750w_80plus_gold', 'corsair_corsair-psu-rm750-80-gold-modular_full01.webp')
                              
    data = mpimage.imread(image_path)
    st.image(data, caption='EDA PSU 750W 80+ Gold')
    df = pd.read_csv('./Analysis/PSU/p_750w_80plus_gold/750w_80plus_gold.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date')

    st.write('# Exploratory Data Analysis untuk komponen PSU 750W 80+ Gold')
    ...
    fig.axes[0].set_title('Data Rata-rata Harga PSU 750W 80+ Gold')
    fig.axes[1].set_title('Tren Rata-rata Harga PSU 750W 80+ Gold')
    ...
    fig.suptitle('Visualisasi ACF dan PACF dari data pelatihan harga rata-rata PSU 750W 80+ Gold', fontsize=16)
    ...
    st.write('- PACF bersifat cuts off setelah lag 3, berarti nilai p = 3, dengan alternatif nilai p adalah 5')
    ...
    perform_adf_test(df_avg_price_train['avg'], series_name="Data Harga Rata-rata PSU 750W 80+ Gold")

    st.subheader('4. Data differencing')

    train_diff = df_avg_price_train.diff().dropna()
    perform_adf_test(train_diff['avg'], series_name="Data Harga Rata-rata PSU 750W 80+ Gold melalui 1x diferensiasi")
