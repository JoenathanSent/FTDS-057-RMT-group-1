...
def run_predict_price():
    ...
    # lightGBM
    lgbm_pct_change = lgbm_predict(group_opt)
    if lgbm_pct_change is not None:
        changes.append(lgbm_pct_change)

    # ARIMA
    arima_pct_change = arima_predict(cat_opt, group_opt)
    ...
    st.write(f'Prediksi perubahan harga dalam 2 bulan ke depan:  {pct_change:.2f}% {"⬆" if pct_change > 0 else "⬇"}')
